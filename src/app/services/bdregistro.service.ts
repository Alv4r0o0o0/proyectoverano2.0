...
export class BdregistroService {
  ...
  ActualizarUsuario(usuario: any) {
    return this.conexionBD.executeSql(
      'UPDATE usuario SET nombreu = ?, apellido = ?, telefono = ?, correo = ?, foto = ? WHERE id_usuario = ?',
      [usuario.nombre, usuario.apellido, usuario.telefono, usuario.correo, usuario.foto, usuario.id_usuario]
    )
      .then(() => {
        this.presentAlert("Perfil actualizado exitosamente");
      })
      .catch(e => {
        this.presentAlert("Error al actualizar el perfil: " + JSON.stringify(e));
